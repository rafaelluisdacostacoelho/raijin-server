...
def status(ctx: ExecutionContext, namespace: str = "supabase") -> None:
    ...
    typer.echo("=" * 60)
    typer.echo("  Verificacoes: 20 itens")
    ...
    typer.echo("\n[18/20] shared_preload_libraries")
    spl_result = run_cmd(
        ["kubectl", "exec", "postgres-0", "-n", namespace, "--",
         "psql", "-U", "postgres", "-t", "-c",
    ...
    # 20. supabase-analytics (Logflare)
    typer.echo("\n[20/20] supabase-analytics (Logflare)")
    analytics_pod = run_cmd(
        ["kubectl", "get", "pods", "-n", namespace, "-l", "app=supabase-analytics",
         "-o", "jsonpath={.items[0].status.phase}"],
        ctx, check=False,
    )
    analytics_phase = (analytics_pod.stdout or "").strip()
    if analytics_phase == "Running":
        typer.secho("  ✓ supabase-analytics rodando.", fg=typer.colors.GREEN)
        # Verify health endpoint
        health_result = run_cmd(
            ["kubectl", "exec", "-n", namespace, "deploy/supabase-analytics", "--",
             "wget", "-qO-", "--timeout=5", "http://localhost:4000/health"],
            ctx, check=False,
        )
        health_body = (health_result.stdout or "").strip()
        if health_result.returncode == 0 and health_body:
            typer.secho("  ✓ Health endpoint /health respondendo.", fg=typer.colors.GREEN)
        else:
            typer.secho("  ⚠ Health endpoint /health sem resposta.", fg=typer.colors.YELLOW)
        # Verify Studio has LOGFLARE_URL env var
        studio_logflare = run_cmd(
            ["kubectl", "get", "deployment", "supabase-studio", "-n", namespace,
             "-o", "jsonpath={.spec.template.spec.containers[0].env[?(@.name=='LOGFLARE_URL')].value}"],
            ctx, check=False,
        )
        logflare_url = (studio_logflare.stdout or "").strip()
        if logflare_url:
            typer.secho(f"  ✓ Studio LOGFLARE_URL={logflare_url}.", fg=typer.colors.GREEN)
        else:
            typer.secho("  ✗ Studio sem LOGFLARE_URL — Log Drains indisponivel!", fg=typer.colors.RED)
        # Verify _supabase database exists
        db_result = run_cmd(
            ["kubectl", "exec", "postgres-0", "-n", namespace, "--",
             "psql", "-U", "postgres", "-t", "-c",
        ...
        db_exists = (db_result.stdout or "").strip()
        if db_exists == "1":
            typer.secho("  ✓ Database _supabase presente.", fg=typer.colors.GREEN)
        else:
            typer.secho("  ✗ Database _supabase NAO encontrado!", fg=typer.colors.RED)
            typer.echo("    Execute: CREATE DATABASE _supabase;")
    elif analytics_phase:
        typer.secho(f"  ⚠ supabase-analytics em estado: {analytics_phase}.", fg=typer.colors.YELLOW)
    else:
        typer.secho("  ✗ supabase-analytics NAO encontrado — Log Drains indisponivel.", fg=typer.colors.RED)
        typer.echo("    Deploy analytics-deployment.yaml para habilitar Log Drains no Studio.")
